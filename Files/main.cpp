...
void main()
{
	...
	fout.open("File.txt", std::ios_base::app);	//2)Открываем поток. При открытии потока мы 
	//уазываем какой именно файл мы будем писать
//std::ios_base::app (append) - дописывать в конец файла 	
	...
	if (fin.is_open())
	{
		...
		// 4) Поток есть смысл закрывать только в том случае,
		// если он был открыт
		fin.close();		
	}
	else
